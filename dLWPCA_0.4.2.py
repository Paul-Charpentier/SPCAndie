...
for i in range(1, len(BERV_Domain)+1):
    plt.axvline(BERV_Domain[i-1], linestyle=':', color='b', alpha=0.5)
plt.show()
...
for i in tqdm(range(1, len(BERV_Domain))):
    ...
    BERV_CORR, stdberv = corrslice(start, end, d2vbinn, sd2vbinn, tbinn, bervbin)
...
fig, axs = plt.subplots(1, 2, sharey=True, tight_layout=True, gridspec_kw={'width_ratios': [10, 1]})

axs[0].imshow(W_rmv, cmap=cm.Greys_r)
axs[0].set_xlabel('$\lambda$ (nm)')
axs[0].set_ylabel('Correlation with BERV > std')
axs[0].set_xlim(0,30)

axs[1].step(sumrvm, range(0,8), where='mid')
axs[1].set_xlabel('%')
plt.show()
...
def filtrbervslice(d2vb, sd2vb, bervb, wbool, bervdom, tbinn=None):
...
d2vfiltr, sd2vfiltr = filtrbervslice(d2vbinn, sd2vbinn, bervbin, W_rmv, BERV_Domain)

##plot before/after

fig, axs = plt.subplots(2, 1, sharey=True, tight_layout=True)

axs[0].plot(d2vbinn[:17].T)
axs[1].plot(d2vfiltr[:17].T)
axs[0].set_xlim(0,69)
axs[1].set_xlim(0,69)
plt.show()
...
ax[0, 1].axhline(fap, linestyle=':', color='k')
ax[0, 1].axvline(74, linestyle=':', color='b', alpha=0.5)
ax[0, 1].set_title('BERV filtered')
...
ax[0, 1].axhline(fap, linestyle=':', color='k')
ax[0, 1].axvline(74, linestyle=':', color='b', alpha=0.5)
ax[0, 1].set_title('filtered + MAD Correction')
